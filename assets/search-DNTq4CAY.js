...
const _sfc_main = {
  ...
  methods: {
    ...
    filterSearch(data, keyword) {
      data = data.filter(
        (v) => v.title.toLowerCase().indexOf(keyword.toLowerCase()) >= 0 || v.sub.toLowerCase().indexOf(keyword.toLowerCase()) >= 0 || v.artist.toLowerCase().indexOf(keyword.toLowerCase()) >= 0 || $(v.content).text().toLowerCase().indexOf(keyword.toLowerCase()) >= 0
      );
      return data;
    },
    toGallery(item) {
...
const search = /* @__PURE__ */ _export_sfc(_sfc_main, [["render", _sfc_render], ["__scopeId", "data-v-011e0493"]]);
export {
  search as default
};
